...
`undo`で取り消したスナップショットを1つ復元します。

```bash
alcom redo
```

- **動作**: `undo`で取り消された直近のスナップショットを復元し、HEADをそのコミットにリセットします。
- **連続実行**: 複数回`undo`した場合、LIFO順で復元されます。
- **制限**: `undo`されていない状態で実行するとエラーになります。`save`を実行するとredo履歴がクリアされます。
...
- **動作**:
    - **セッションが存在する場合**（一時スナップショットがある場合）:
        1.  未コミット変更をステージし、現在のインデックスからツリーオブジェクトを作成します。
        2.  そのツリーとセッション開始前のコミットを親として、新しいコミットを直接作成します（`commit-tree`）。`commit.gpgsign=true` 設定を検出した場合は `-S` フラグを付与し、ユーザーの署名設定を反映します。
        3.  内部状態をクリアし、ブランチを新しいコミットに更新します。
